...
\begin{equation}
    ...
    \psi \approx \tan \psi \approx \frac{M_2}{M_1 + M_2} \, \theta
...
%
respectively.
Like $\theta$, $\psi$ and $\Delta E$ are proportional to $1/E$.
Interestingly, $\psi$ does not depend on the atom masses $M_1$ and $M_2$.
...
\begin{equation}
    ...
    \psi \approx \frac{Z_1 Z_2 e^2}{a_\mathrm{I}} \, \frac{I(P)}{E}
\end{equation}
%
and
%
...
With a purely repulsive interatomic potential, increasing impact parameters $P$ result in decreasing scattering angles $\psi$, so according to \cref{eq:sn_impulse_psi_I}, $I(P)$ should be a monotonically decreasing function.%
%
...
Thus, if we want to guarantee that by choosing the impact parameter between $0$ and $P_\mathrm{max}$ we do not miss any collisions with either $\psi > \psi_\mathrm{min}$ or $\Delta E > \Delta E_\mathrm{min}$, both $\psi(P_\mathrm{max}) \le \psi_\mathrm{min}$ and $\Delta E(P_\mathrm{max}) \le \Delta E_\mathrm{min}$ must hold.
...
On the other hand, for a given $P_\mathrm{max}$, conditions for $E$ result from $\psi(P_\mathrm{max}) \le \psi_\mathrm{min}$ and $\Delta E(P_\mathrm{max}) \le \Delta E_\mathrm{min}$:
%
\begin{equation}
...
\begin{figure}
    \centering
    \includegraphics[scale=0.5]{impulse_He_W.pdf} ~~
    \includegraphics[scale=0.5]{impulse_As_Si.pdf}
