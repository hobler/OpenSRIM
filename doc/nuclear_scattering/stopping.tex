...
\begin{figure}
    \centering
    \includegraphics[scale=0.5]{sn_ratio_all_zbl_at_rp10.pdf} ~~
    \includegraphics[scale=0.5]{sn_ratio_all_krc_at_rp10.pdf}
    \caption{Ratio of nuclear stopping cross sections for all atom pair using the NLHlin potential and the ZBL potential (left) of KrC potential (right) at energies that lead to a projected range of 10~\AA, but at least 10~eV.}
    \label{fig:sn_sn_ratio_at_rp}
\end{figure}
%
As an application, stopping cross sections have been calculated for all atom pairs using the analytic expression \cref{eq:sn_sn_fit} at energies that cause a projected range of 10~\AA. For the calculation of the projected range, the algorithm described in the next section has been used. The ratio of the results using the NLHlin potential and the ZBL or KrC potential are shown in \cref{fig:sn_sn_ratio_at_rp}.