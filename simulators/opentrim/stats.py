...
def init_stats(NELEM_ION, NELEM_TARGET, input_params):
    ...
    nvar = {
        "i": NELEM + NELEM_TARGET if follow_recoils else NELEM,
    ...
    STATS_DTYPE = None
    # yields (0d distributions)
    for short_name in ("i", "b", "t"):
        stats_dtype = np.dtype([
            ("score", np.int64),  # whether to score this statistics 
            ("nvar", np.int32),  # number of variables (e.g. atom species)
    ...
    for short_name in stats_configs:
        stats_config = stats_configs[short_name]
        if not short_name.startswith("xy"):  # 1d
            stats_dtype = np.dtype([
                ("score", np.int64),  # whether to score this statistics 
                                      # (use integer for JIT compatibility)
                ("nvar", np.int32),  # number of variables (e.g. atom species)
                ("nbins", np.int32),  # number of bins
                ("limits", np.float64, (2,)),
    ...
    for short_name, stats_config in stats_configs.items():
        ...
        if not short_name.startswith("xy"):  #1d
            ...
            for field in stats_config:
                if field not in ["score", "x_nbins", "y_nbins", 
                                 "x_limits", "y_limits"]:
                    raise ValueError(f"Unknown histogram config field: {field} "
                                    f"in {short_name}")
                stats[0][short_name][field] = stats_config[field]
    ...
    #print(f"stats field names: '{STATS_DTYPE.names}'")
...
def zero_stats(stats):
    ...
    """
    for field_name in STATS_DTYPE.names:
        if "counts" in stats[field_name].dtype.names:
            stats[field_name]["counts"].fill(0.0)
        if "power_sums" in stats[field_name].dtype.names:
            stats[field_name]["power_sums"].fill(0.0)
...
def merge_stats(total_stats, stats):
    ...
    """
    for field_name in STATS_DTYPE.names:
        if "power_sums" in total_stats[field_name].dtype.names:
            total_power_sums = total_stats[field_name]["power_sums"]
            power_sums = stats[field_name]["power_sums"]
            total_power_sums += power_sums
        if "counts" in total_stats[field_name].dtype.names:
            total_counts = total_stats[field_name]["counts"]
            counts = stats[field_name]["counts"]
            total_counts += counts
...
def _score0d(stats_field, value, ivar):
    ...
    """
    stats_field["counts"][ivar] += 1.0
    for i in range(2*max_order + 1):
        stats_field["power_sums"][ivar, i] += value ** i
...
def _score1d(stats_field, value, ivar, weight=1.0):
    ...
    if stats_field["score"]:
        for i in range(2*max_order + 1):
            stats_field["power_sums"][ivar, i] += weight * value ** i

        if value < stats_field["limits"][0]:
            ibin = 0  # Underflow bin
        elif value < stats_field["limits"][1]:
            ibin = int((value - stats_field["limits"][0]) / 
                        stats_field["bin_width"]) + 1
        else:
            ibin = -1  # Overflow bin
        stats_field["counts"][ivar, ibin] += weight
...
def _score2d(stats_field, x, y, ivar, weight=1.0):
    ...
    Arguments:
        ...
        y: The y-coordinate to be scored
        ivar: The index of the variable (e.g. atom species) for which to score
        weight: The weight of this contribution (default 1.0)
    """
    if stats_field["score"]:
        if x < stats_field["x_limits"][0]:
            ix = 0
        elif x < stats_field["x_limits"][1]:
            ix = int((x - stats_field["x_limits"][0]) /
                     stats_field["x_bin_width"]) + 1
        else:
            ix = -1

        if y < stats_field["y_limits"][0]:
            iy = 0
        elif y < stats_field["y_limits"][1]:
            iy = int((y - stats_field["y_limits"][0]) /
                     stats_field["y_bin_width"]) + 1
        else:
            iy = -1
        stats_field["counts"][ivar, ix, iy] += weight
...
@jit(debug=config.DEBUG)
def score_yield_in(stats, nin):
    """Score the yield of projectiles stopped/started in the target."""
    for ivar, nin_val in enumerate(nin):
        _score0d(stats["i"], nin_val, ivar)

        
@jit(debug=config.DEBUG)
def score_yield_back(stats, nback):
    """Score the yield of backscattered projectiles."""
    for ivar, nback_val in enumerate(nback):
        _score0d(stats["b"], nback_val, ivar)

        
@jit(debug=config.DEBUG)
def score_yield_trans(stats, ntrans):
    """Score the yield of transmitted projectiles."""
    for ivar, ntrans_val in enumerate(ntrans):
        _score0d(stats["t"], ntrans_val, ivar)

        
def standardize_moments(stats_field, ivar):
    ...
    # Counts
    power_sums = stats_field["power_sums"][ivar, :]
    count = power_sums[0]
    std_moments[0] = count
    if count == 0:
